...
import CommunicationOld.I2Cdev;
 ...
 * @author PerEspen
 */
public class CommunicationOld implements Runnable
{
    ...
    private byte[] read(I2CDevice deviceUsed, byte[] buffer, int byteSize, byte registerAddr)
    {
        ...
        {
            Logger.getLogger(CommunicationOld.class.getName()).log(Level.SEVERE, null, ex);
        }
        return buffer;
    }
   ...
   /*private void requestSensorData(I2CDevice device)
    {
       ...
       {
           read(device, dataBuffer, stdByteSize, SENSORDATA_C1_ADDR);
       }
       else if(device.getAddress() == i2cdevices.ARD_ROBOT_ADDR)
       {
           read(device, dataBuffer, stdByteSize, SENSORDATA_C2_ADDR);
       }
